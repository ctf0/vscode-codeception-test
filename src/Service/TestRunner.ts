...
import { ExecutionError, type TestStatus } from '../Util/ExecutionError';
...
export class TestRunner {
                ...
                try {
                    ...
                    if (stdout) {
                        ...
                        if (status.failed > 0) {
                            ...
                            if (status.successful !== test.children.size) {
                                this.testFail(run, test, status, testData, request);
                            } else {
    ...
    private testError(run: vscode.TestRun, test: vscode.TestItem, testData: { className: string; method?: string; configFile?: string; }, request: vscode.TestRunRequest) {
    ...
    private testSkip(run: vscode.TestRun, test: vscode.TestItem, testData: { className: string; method?: string; configFile?: string; }, request: vscode.TestRunRequest) {
        run.skipped(test);
    ...
    private testPass(run: vscode.TestRun, test: vscode.TestItem, testData: { className: string; method?: string; configFile?: string; }, request: vscode.TestRunRequest) {
        run.passed(test);
    ...
    private testFail(run: vscode.TestRun, test: vscode.TestItem, status: TestStatus, testData: { className: string; method?: string; configFile?: string; }, request: vscode.TestRunRequest) {
